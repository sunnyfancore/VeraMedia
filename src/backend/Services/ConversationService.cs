    ...
    private static string BuildUserContent(SendMessageRequest request)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Options?.Capability))
        {
            ...
            if (string.Equals(request.Options.Capability, "ppt", StringComparison.OrdinalIgnoreCase))
            {
                ...
                builder.AppendLine("{\"title\":\"整套 PPT 标题\",\"subtitle\":\"一句话副标题\",\"audience\":\"受众\",\"theme\":\"自定义高级主题名\",\"design\":{\"style\":\"整体视觉风格与质感\",\"palette\":\"色彩、光感、材质建议\",\"motif\":\"贯穿全稿的视觉母题\",\"composition\":\"版式语言与画面节奏\"},\"slides\":[{\"title\":\"页标题\",\"subtitle\":\"可选副标题\",\"layout\":\"AI 自创版式标签，例如 cinematic-opening / hero-metric / 2x2-risk-matrix / dashboard-panel / split-narrative\",\"bullets\":[\"页面短要点1\",\"页面短要点2\"],\"visual\":\"这一页具体如何画：主视觉、图表、卡片、流程、对比矩阵、场景图、视觉隐喻、动线和留白\",\"imageUrl\":\"如果有可用的图片 URL 则填入，系统将自动嵌入该图片到幻灯片中\",\"notes\":\"演讲备注或旁白\"}]}");
                builder.AppendLine("```");
                ...
                builder.AppendLine("- PPT 页面内容、备注和可见文案不要出现平台品牌名或固定工作区脚注，除非用户明确要求。");
                builder.AppendLine();
                ...
                builder.AppendLine("- layout 不从固定列表里选择；请为每页自创语义化版式名。后端会结合 layout 与 visual brief 解释成主视觉页、矩阵页、仪表盘页、大数字页、流程页、对比页、杂志式页等不同构图。");
                builder.AppendLine("- visual 必须说明真实版式形态，包括主体图形、信息区比例、视觉重心、图表结构、卡片/留白/动线关系；不要只写抽象风格词。");
                builder.AppendLine();
                builder.AppendLine("## 版式节奏");
                builder.AppendLine("- 避免一页页罗列。每 2-3 页要有节奏变化：大标题页、视觉冲击页、数据页、结构页、结论页交替出现。");
                builder.AppendLine("- 允许大胆留白、强对比、大数字、全幅场景、杂志式标题、咨询级框架图、发布会式视觉焦点；每页都可以设计不同版式形态，不必复用上一页模板。");
